...
import type { Clock, CoreRuntime, EmailProvider, IdGen, PaymentProvider, StorageProvider } from '@tenantkit/kernel'
...
export interface SupabaseRuntimeOptions {
  /** REQUIRED — transactional email (e.g. @tenantkit/email-resend). Supabase doesn't own your templates. */
  email: EmailProvider
  /** A writable cookie adapter factory for the current request. @tenantkit/next supplies one over next/headers. */
  cookies: () => Promise<CookieAdapter>
  /** OPTIONAL — the payments plugin's provider (e.g. @tenantkit/payments-stripe). */
