 ...
 * and the SSR client propagates the refreshed `Set-Cookie` headers. The @tenantkit/next middleware calls this
 * on every request.
 */
import type { AuthSession, SessionStore } from '@tenantkit/kernel'
...
function serializeCookie(name: string, value: string, options?: Record<string, unknown>): string {
  // Minimal serializer; the @tenantkit/next binding uses Next's cookie API in production.
