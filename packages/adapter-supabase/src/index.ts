/**
 * @tenantkit/adapter-supabase — Supabase reference adapter for the kernel ports.
 *
 * Most apps need only `createSupabaseRuntime()`. The individual factories are exported for partial use
 * (e.g. keep Supabase for DB but bring your own IdentityProvider). Provisional scope `@tenantkit/*` (ADR-0010).
 */
