...
import type { ZodErrorMap } from '../i18n/zod-locale'
...
/**
 * Parse + validate a JSON request body. A malformed JSON payload becomes a 400 BAD_REQUEST. Pass `errorMap`
 * (e.g. `zodErrorMap(locale)`) to localize Zod's built-in messages; `withRoute` supplies it from `ctx.locale`.
 */
export async function parseJson<T>(req: Request, schema: ZodSchema<T>, errorMap?: ZodErrorMap): Promise<ParseResult<T>> {
  let raw: unknown
  try {
    raw = await req.json()
  } catch {
  ...
  const result = schema.safeParse(raw, errorMap ? { error: errorMap } : undefined)
...
export function parseQuery<T>(req: Request, schema: ZodSchema<T>, errorMap?: ZodErrorMap): ParseResult<T> {
  ...
  const result = schema.safeParse(obj, errorMap ? { error: errorMap } : undefined)
  return result.success ? { success: true, data: result.data } : fail(result.error)
}

/** Parse + validate selected request headers (e.g. an idempotency key, a tenant host). */
export function parseHeaders<T>(req: Request, schema: ZodSchema<T>, errorMap?: ZodErrorMap): ParseResult<T> {
